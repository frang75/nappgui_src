    ...
    if (self->listeners.OnDraw != NULL)
    {
        ...
        if ((self->flags & ekVIEW_OPENGL) == 0)
        {
            ...
            if (nscontext != nil)
            {
                ...
                params.ctx = self->ctx;
                dctx_set_gcontext(self->ctx, nscontext, rect.size.width, rect.size.height, 96, 1.f, params.x, params.y, 0, (self->flags & ekVIEW_CONTROL) ? FALSE : TRUE);
                ...
                if (self->OnOverlay != NULL)
                {
                    params.x = 0;
                    params.y = 0;
                    dctx_set_gcontext(self->ctx, nscontext, rect.size.width, rect.size.height, 96, 1.f, 0, 0, 0, (self->flags & ekVIEW_CONTROL) ? FALSE : TRUE);
