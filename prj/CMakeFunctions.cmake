...
function(processTarget targetName targetType dependList)
    ...
    elseif (targetType STREQUAL DYNAMIC_LIB)
        ...
        get_filename_component(targetSourceUpper ${targetSource} NAME)
        string(TOUPPER ${targetSourceUpper} targetSourceUpper)
        set_property(TARGET ${targetName} APPEND PROPERTY COMPILE_DEFINITIONS NAPPGUI_${targetSourceUpper}_EXPORT_DLL)
...
function(targetLinkWithLibraries targetName firstLevelDepends)
    ...
    if (${targetName}_LINKDEPENDS)
        ...
        foreach(depend ${${targetName}_LINKDEPENDS})
            ...
            # If target depends on shared library, we define the 'NAPPGUI_target_IMPORT_DLL' preprocessor macro
            if (DEPEND_IS_SHAREDLIB)
                targetSourceDir(${dependName} targetSource)
                get_filename_component(targetSourceUpper ${targetSource} NAME)
                string(TOUPPER ${targetSourceUpper} targetSourceUpper)
                set_property(TARGET ${targetName} APPEND PROPERTY COMPILE_DEFINITIONS NAPPGUI_${targetSourceUpper}_IMPORT_DLL)
            endif()
...
function(createLibFiles libPath dependencies)
    ...
    file(APPEND ${libPathName}.def "#if defined(__GNUC__) || defined(__clang__)\n")
